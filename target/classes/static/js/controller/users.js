    ...
    function ($scope, $rootScope, $http, $location, $filter,$window, Notification) {
        ...
        $scope.saveuserDetails = function () {
            if ($scope.userText == ''||$scope.userText==null||angular.isUndefined($scope.userText)) {
                ...
                $http.post($scope.taskServerURL + "/getuserDetails" , angular.toJson(saveUserDetails)).then(function (response) {
                    var data = response.data;
                    if (data == "" || data == 'undefined') {
                        Notification.error({message: 'User Already exists', positionX: 'center', delay: 2000});
                    } else {
        ...
        $scope.getUserList = function (searchText) {
            ...
            $http.post($scope.taskServerURL + '/getUserList').then(function (response) {
        ...
        $scope.getPaginatedUsersList = function (page) {
            ...
            $http.post($scope.taskServerURL + '/getPaginatedUsersList?searchText='+$scope.searchText,angular.toJson(paginationDetails)).then(function (response) {
                var data = response.data;
                console.log(data);
        ...
        $scope.DeleteUser=function(data) {
            bootbox.confirm({
                ...
                callback: function (result) {
                    if (result == true) {
                        $http.post($scope.taskServerURL + '/getDeleteUser?useraccountId=' + data.useraccountId).then(function (response, status, headers, config) {
                            var data = response.data;
